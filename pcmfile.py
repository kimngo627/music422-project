...
class PCMFile(AudioFile):
    ...
    def ReadFileHeader(self):
        ...
        tag = self.fp.read(12)
        if tag[0:4] != b"RIFF" or tag[8:12]!= b"WAVE":  raise RuntimeError("ERROR: File opened for PCMFile is not a RIFF file!")
        # find format chunk
        while True:
             tag=self.fp.read(4)
             if len(tag)<4: raise RuntimeError("ERROR: Didn't find WAV file 'fmt ' chunk following RIFF file header")
             if tag[0:4] == b"fmt ":  break
    ...
    def ReadDataBlock(self,codingParams):
        ...
        if codingParams.bitsPerSample == 16:
            ...
            dataBlock=unpack(b"<"+str(codingParams.nSamplesPerBlock*codingParams.nChannels).encode()+b"h",dataBlock)  # asumes nSamples*nChannels SIGNED short ints
#            dataBlock=np.fromstring(dataBlock,dtype=np.int16)  # uses Local Endian conversion -- use byteswap() method if wrong Endian
        else: raise IOError("PCMFile was not 16-bit PCM in PCMFile.ReadDataBlock!")
    ...
    def WriteFileHeader(self,codingParams):
        ...
        self.fp.write(  pack('<4sL4s4sLHHLLHH4sL', \
            b"RIFF", chunkSize, b"WAVE", b"fmt ", formatSize, formatTag, codingParams.nChannels, codingParams.sampleRate, \
              bytesPerSec, blockAlign, codingParams.bitsPerSample, b"data", dataBytes )  )
    ...
    def WriteDataBlock(self,data,codingParams):
        ...
        nChannels = len(data)
        if nChannels != codingParams.nChannels: raise RuntimeError("Data block to PCMFile did not have expected number of channels")
 ...
 #           for i in range(len(dataBlock)):  dataString += pack('<h',dataBlock[i])
        ...
        else: raise IOError("Asked to write to a PCM file with other than 16-bits per sample in PCMFile.WriteDataBlock!")
